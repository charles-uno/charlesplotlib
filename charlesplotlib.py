...
class plotcell(object):
    ...
    def imax(self, i):
        ...
        """
        bmax = nax( (x, y, None)[i] for x, y, a, k in self.bars )
        cmax = nax( (x, y, z)[i] for x, y, z, a, k in self.contours )
        lmax = nax( (x, y, None)[i] for x, y, a, k in self.lines )
        mmax = nax( (x, y, z)[i] for x, y, z, a, k in self.meshes )
        return nax(bmax, cmax, lmax, mmax)
    ...
    def imed(self, i):
        ...
        lines, and meshes this plot holds.
        """
        bmed = ned( (x, y, None)[i] for x, y, a, k in self.bars )
        cmed = ned( (x, y, z)[i] for x, y, z, a, k in self.contours )
        lmed = ned( (x, y, None)[i] for x, y, a, k in self.lines )
        mmed = ned( (x, y, z)[i] for x, y, z, a, k in self.meshes )
        return ned(bmed, cmed, lmed, mmed)
    ...
    def imin(self, i):
        ...
        lines, and meshes this plot holds.
        """
        bmin = nin( (x, y, None)[i] for x, y, a, k in self.bars )
        cmin = nin( (x, y, z)[i] for x, y, z, a, k in self.contours )
        lmin = nin( (x, y, None)[i] for x, y, a, k in self.lines )
        mmin = nin( (x, y, z)[i] for x, y, z, a, k in self.meshes )
        return nin(bmin, cmin, lmin, mmin)
...
_savepath = os.environ['HOME'] + '/Desktop/plots/' + now()
